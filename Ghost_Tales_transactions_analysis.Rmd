...
ggplot(RFM, aes(as.numeric(recency))) + geom_histogram(color = "steelblue4", fill = "steelblue") +
    labs(title = "Historgam of recency", x = "Recency (days)") +
...
```
Average Lifespan is **`r avgLifespan`** days. Average Paying lifecycle is **`r avgPayingLifecycle`** days. Average period to first transaction is **`r avgPeriodToFirstTrans`** days.
...
Average paying lifecycle ~ `r avgPayingLifecycle` days, calculate Retention Rate per month (Purchase cycle):
```{r}
...
```
Average Retention Rate is **`r avgRetentionRate`%**.
...
ggplot(dCohortsInst, aes(as.Date(first_install_date), avg_number_of_transaction)) +
    ...
    labs(x = "Install date", y = "Average number of transactions") +
    scale_x_date(breaks = pretty_breaks(12)) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
